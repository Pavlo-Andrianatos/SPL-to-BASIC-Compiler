...
public class Node {
    ...
    public void writeToBASICPhaseOne(Node node, FileWriter writer) throws IOException {
        ...
        if(node.name.equals("COND_BRANCH")){
            ...
            if(node.children.size() > 4 && node.children.get(4).children.get(0).name.equals("else")){
                ...
                writer.write(basicLineNumber + " GOTO G_" + tempGoToNumber);
                basicLineNumber += 10;
                writer.write("\n");

                writeToBASICPhaseOne(node.children.get(3), writer);
                writer.write("G_" + tempGoToNumber + " " + " ife");
                node.children.get(3).writtenToBASIC = true;
                node.children.get(4).writtenToBASIC = true;
                node.children.get(4).children.get(0).writtenToBASIC = true;
            } else{
                writeToBASICPhaseOne(node.children.get(3), writer);
        ...
        if(node.name.equals("BOOL")){
            if(node.children.get(0).name.equals("and")){
                if(node.children.get(1).children.get(0).name.equals("VAR")){
                    ...
                    writer.write(basicLineNumber + " ANDTEMPCOMP = " + node.children.get(1).children.get(0).children.get(0).name + " * " + node.children.get(2).children.get(0).children.get(0).name);
                    basicLineNumber += 10;
                    writer.write("\n");

                    writer.write(basicLineNumber + " ANDCOMP = ANDTEMPCOMP = ONE");
                    basicLineNumber += 10;
                    writer.write("\n");
                ...
                //if(node.children.get(1).name.equals("VAR")){
                    ...
                    node.children.get(1).children.get(0).writtenToBASIC = true;
                    node.children.get(2).writtenToBASIC = true;
                    node.children.get(2).children.get(0).writtenToBASIC = true;
                //} else{
                    //writeToBASICPhaseOne(node.children.get(1), writer);
                    //node.children.get(1).writtenToBASIC = true;
                //}
        ...
        if(node.name.equals("COND_LOOP")){
            if(node.children.get(0).name.equals("for")){
                ...
                writer.write(basicLineNumber + " GOTO G_" + tempGoToNumber + " for");
                basicLineNumber += 10;
                writer.write("\n");
                node.children.get(0).writtenToBASIC = true;
                node.children.get(1).writtenToBASIC = true;
                node.children.get(2).writtenToBASIC = true;
                node.children.get(3).children.get(1).writtenToBASIC = true;
                node.children.get(5).writtenToBASIC = true;
                node.children.get(4).writtenToBASIC = true;
            }
    ...
    public void writeToBASICPhaseTwo(String stringBASIC) throws IOException {
        ...
        int rangeProc = 1;
        int tempGoToNumber = gotoNumber;
        int tempGoToProc = gotoProc;
        while(tempGoToNumber > 10){
            tempGoToNumber = tempGoToNumber/10;
            rangeNumber += 1;
        }
        while(tempGoToProc > 10){
            tempGoToProc = tempGoToProc/10;
            rangeProc += 1;
        }
        ...
        for (int i = 0; i < lines.length; i++) {
            if(lines[i].contains("G_") && lines[i].contains("for")){
                ...
                /*for(int j = i-1; j >= 0; j--){
                    ...
                    if(lines[j].contains("GOTO " + gotoCheck)){
                        firstLineOfProc = (j * 10) + 20;
                        break;
                    }
                }*/
                for(int j = i-1; j >= 0; j--){
                ...
                for(int j = i-1; j >= 0; j--){
                    if(lines[j].contains(gotoCheck)){
                        if(lines[j].contains("IF")){
                            ...
                            lines[j] = lines[j].replace(gotoCheck, updatedLine + "");
                        } else{
                            lines[i] = lines[i].replace(gotoCheck, "");
                            lines[i] = lines[i].replace("ife", "");
                            lines[i] = lines[i].trim();
