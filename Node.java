    ...
    public void writeToBASICPhaseOne(Node node, FileWriter writer) throws IOException {
        ...
        if(node.name.equals("COND_LOOP")){
            if(node.children.get(0).name.equals("for")){
                ...
                writer.write(basicLineNumber + " IF VARIABLECOMP THEN GOTO G_" + gotoNumber);
                basicLineNumber += 10;
                writer.write("\n");
                ...
                writer.write(basicLineNumber + " GOTO G_" + gotoNumber + " for");
                basicLineNumber += 10;
                writer.write("\n");
                gotoNumber += 1;
            }
            if(node.children.get(0).name.equals("while")){
                ...
                writer.write(basicLineNumber + " GOTO G_" + gotoNumber + " while");
                basicLineNumber += 10;
                writer.write("\n");
                gotoNumber += 1;
            }
    ...
    public void writeToBASICPhaseTwo(String stringBASIC) throws IOException {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if(lines[i].contains("P") && lines[i].contains("RETURN")){
                String gotoCheck = lines[i].substring(lines[i].indexOf("P"), (lines[i].lastIndexOf("P") + 2));
